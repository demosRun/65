...
function getScale () {
  ...
  switch (autoScaleInfo.type) {
    ...
    case 'scaleH': {
      ...
      for (var index = 0; index < boxList.length; index++) {
        ...
        if ((autoScaleInfo.innerWidth / autoScaleInfo.innerHeight) < 1) {
          var scale = autoScaleInfo.innerHeight / autoScaleInfo.deviseH
          autoScaleInfo.scale = scale
          scaleBox.style.width = autoScaleInfo.deviseW + 'px'
          if (autoScaleInfo.scroll) {
            scaleBox.style.height = autoScaleInfo.innerHeight / autoScaleInfo.scale + 'px'
          ...
          // 判断缩放中心
          if (autoScaleInfo.center === 'middle') {
            scaleBox.style.transform = 'scale(' + scale + ', ' + scale + ') translate(' + autoScaleInfo.hideWidth + 'px, 0)';
          } else {
            scaleBox.style.transform = 'scale(' + scale + ', ' + scale + ')';
          }
    ...
    // 山下左右保证单屏PC显示
    case 'scalePC': {
      var scaleH = autoScaleInfo.innerHeight / autoScaleInfo.deviseH
      var scaleW = autoScaleInfo.innerWidth / autoScaleInfo.deviseW
      autoScaleInfo.scale = scaleH > scaleW ? (scaleH).toFixed(2) : (scaleW).toFixed(2)
      for (var index = 0; index < boxList.length; index++) {
        var scaleBox = boxList[index];
        scaleBox.parentNode.style.width = '100%'
        scaleBox.parentNode.style.height = '100%'
        scaleBox.parentNode.style.overflow = 'hidden'
        scaleBox.style.width = autoScaleInfo.deviseW + 'px'
        scaleBox.style.height = autoScaleInfo.deviseH + 'px'
        scaleBox.style.overflow = 'hidden'
        scaleBox.style.transform = 'scale(' + autoScaleInfo.scale + ', ' + autoScaleInfo.scale + ') translate(' + (autoScaleInfo.innerWidth - autoScaleInfo.deviseW * autoScaleInfo.scale) / 2 / autoScaleInfo.scale + 'px, ' + (autoScaleInfo.innerHeight - autoScaleInfo.deviseH * autoScaleInfo.scale) / 2 / autoScaleInfo.scale + 'px)'
        scaleBox.style.transformOrigin = '0 0 0'
        if (autoScaleInfo.scroll) {
          scaleBox.style.overflow = 'auto'
        }
    ...
    case 'rotateH5': {
      for (var index = 0; index < boxList.length; index++) {
        ...
        if ((autoScaleInfo.innerWidth / autoScaleInfo.innerHeight) < 1) {
          ...
          var deviseScale = autoScaleInfo.deviseW / autoScaleInfo.deviseH
          // PC保证高度铺满宽度自适应
          var scale = window.innerHeight / autoScaleInfo.deviseH
          autoScaleInfo.hideHeight = (autoScaleInfo.innerHeight - autoScaleInfo.deviseH * scale) / 2 /scale
          autoScaleInfo.hideWidth = (autoScaleInfo.innerWidth - autoScaleInfo.deviseW * scale) / 2 /scale
          autoScaleInfo.zoom = scale
  ...
  if (window.autoScalecallBack) {
    window.autoScalecallBack(autoScaleInfo)
  }
  setTimeout(function () {
    window.autoScaleFinish = true
  }, 200);
